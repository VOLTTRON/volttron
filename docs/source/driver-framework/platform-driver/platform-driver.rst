...
.. code-block:: bash
    ...
    python scripts/install-agent.py -s services/core/PlatformDriver -c <platform driver config file>
...
`services/core/PlatformDriver/platform-driver.json`.
...
Consider the following three configuration files:  A platform driver configuration called `platform-driver.json`, a
...
.. code-block:: bash
    ...
    volttron-ctl config store platform.driver config platform-driver.json
