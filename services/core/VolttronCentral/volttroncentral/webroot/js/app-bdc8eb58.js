...
devicesStore.dispatchToken = dispatcher.register(function (action) {
    ...
    function loadDevice(data, platform, bacnetUuid) {
        if (data) {
            ...
            _devices[device.device_id.toString()] = {
                ...
                selectedPoints: [],
                items: [{ key: "address", label: "Address", value: device.address }, { key: "deviceId", label: "Device ID", value: device.device_id.toString() }, { key: "vendorId", label: "Vendor ID", value: device.vendor_id }, { key: "vendor", label: "Vendor", value: vendorTable[device.vendor_id] }, { key: "type", label: "Type", value: "BACnet" }]
            };
        }
    }
});
