...
var platformActionCreators = {
    ...
    loadAgents: function (platform) {
        ...
        new rpc.Exchange({
            ...
            .then(function (agentsList) {
                platform.agents = agentsList.map(function (agent) {
                    agent.tag = (agent.tag? agent.tag : agent.name);
    ...
    removeAgent: function (platform, agent) {
        ...
        new rpc.Exchange({
            ...
            .then(function (result) {
                ...
                {
                    platformActionCreators.updatePlatform(platform);
                }
            })                      
            .catch(rpc.Error, function (error) {
    ...
    installAgents: function (platform, files) {
        ...
        console.log("installingAgents: " + platform.agents.length + " agents");
        ...
        new rpc.Exchange({
            ...
            .then(function (results) {
                ...
                console.log("agents installed: " + platform.agents.length + " agents");
    ...
    updatePlatform: function (platformToUpdate) {
        // var authorization = authorizationStore.getAuthorization();
        ...
        //             if (platform.uuid === platformToUpdate.uuid)
        //             {
        //                 platformActionCreators.updateAgents(platform);
        //             }                    
        //         });
        //     })
        //     .catch(rpc.Error, function (error) {
        ...
        platformActionCreators.updateAgents(platformToUpdate);
    },
    updateAgents: function (platform) {
        var authorization = authorizationStore.getAuthorization();
        ...
        new rpc.Exchange({
            ...
            .then(function (agentsList) {
                agentsList = agentsList.map(function (agent) {
                    agent.tag = (agent.tag ? agent.tag : agent.name);
                ...
                dispatcher.dispatch({
                    type: ACTION_TYPES.UPDATE_PANEL_PLATFORM,
                    platform: platform,
                    agents: agentsList
                });

                platform.agents = JSON.parse(JSON.stringify(agentsList));

                console.log("dispatch receive platform: " + platform.agents.length + " agents");
                dispatcher.dispatch({
                    type: ACTION_TYPES.RECEIVE_PLATFORM,
                    platform: platform,
                });                
                ...
                new rpc.Exchange({
                    ...
                    .then(function (agentStatuses) {
                        ...
                        console.log("agentStatuses length: " + agentStatuses.length);
                        console.log("dispatch update platform: " + platform.agents.length + " agents");
...
var platformsPanelActionCreators = {    
    ...
    loadChildren: function(type, parent)
    {
        ...
        function updatePanelAgents(platform) {
            var authorization = authorizationStore.getAuthorization();
...
var AgentRow = React.createClass({displayName: "AgentRow",
    ...
    render: function () {
        ...
        return (
            React.createElement("tr", {key: agent.uuid}, 
...
var Platform = React.createClass({displayName: "Platform",
    ...
    render: function () {
        ...
        if (platform && platform.hasOwnProperty("agents"))
        {
            console.log("render platform: " + platform.agents.length + " agents");
        }
...
platformsPanelItemsStore.dispatchToken = dispatcher.register(function (action) {
    ...
    switch (action.type) {
        ...
        case ACTION_TYPES.UPDATE_PANEL_PLATFORM:

            var uuid = action.platform.uuid;

            if (_items.platforms.hasOwnProperty(uuid))
            {
                var agentsIndex = _items.platforms[uuid].children.indexOf("agents");

                var expanded = false;

                //initialize the platform's agents list by removing it
                if (agentsIndex > -1)
                {
                    expanded = _items.platforms[uuid].agents.expanded;

                    _items.platforms[uuid].children.splice(agentsIndex, 1);
                    delete _items.platforms[uuid].agents;
                }

                if (action.agents.length > 0)
                {
                    insertAgents(_items.platforms[uuid], action.agents, expanded);
    ...
    function insertAgents(platform, agents, expanded)
    {
        ...
        platform.agents.expanded = (expanded ? true : false);
        ...
        agentsToInsert.forEach(function (agent)
        {
            ...
            agentProps.name = (agentProps.tag ? agentProps.tag : agentProps.name);
...
platformsStore.getPlatform = function (uuid) {
    ...
    if (foundPlatform && foundPlatform.hasOwnProperty("agents"))
    {
        console.log("foundPlatform: " + foundPlatform.agents.length + " agents");
    }
...
platformsStore.dispatchToken = dispatcher.register(function (action) {
    ...
    switch (action.type) {
        ...
        case ACTION_TYPES.UPDATE_PLATFORM:

            var match = false;

            if (action.platform.hasOwnProperty("agents"))
            {
                console.log("store update platform: " + action.platform.agents.length + " agents");
            }
            
            _platforms = _platforms.map(function (platform){

                if (platform.uuid === action.platform.uuid)
                {
            ...
            if (match)
            {  
                if (_platforms[0].hasOwnProperty("agents"))
                {
                    console.log("after platforms map: " + _platforms[0].agents.length + " agents");
                }
