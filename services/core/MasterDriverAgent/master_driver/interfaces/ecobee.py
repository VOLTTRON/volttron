...
class Interface(BasicRevert, BaseInterface):
    ...
    def configure(self, config_dict, registry_config_str):
        ...
        ecobee_id = self.config_dict.get('DEVICE_ID')
        if not isinstance(ecobee_id, int):
            try:
                self.ecobee_id = int(ecobee_id)
            except ValueError:
                raise ValueError(
    ...
    def get_auth_config_from_store(self):
        return json.loads(self.vip.rpc.call(CONFIGURATION_STORE, "manage_get", PLATFORM_DRIVER, self.auth_config_path))

    def get_ecobee_data(self):
        """
        ...
        try:
            ...
            try:
                ...
                _log.error("Failed to obtain Ecobee data from cache after refreshing tokens, obtaining new "
                           "authorization code")
                # TODO do we update auth and try again, or throw an error
                self.authorization_stage = "UNAUTHORIZED"
                self.update_authorization()
    ...
    def _scrape_all(self):
        ...
        registers = holds + settings + [programs_register, vacations_register, status_register]
