...
class Interface(BasicRevert, BaseInterface):
    ...
    def request_pin(self):
        ...
        self.pin = response_json['ecobeePin']
        _log.warning("***********************************************************")
        _log.warning(
    ...
    def request_tokens(self):
        ...
        try:
            ...
            _log.error(re)
            _log.warning(
                "Error connecting to Ecobee. Possible connectivity outage. Could not request tokens.".format(
                    response.status_code))
            return
        self.authenticated = True
        response_json = json.loads(response.content)
    ...
    def handle_response_code(response_code, text):
        ...
        :param response_code: HTTP request response code
        :param text: HTTP request response text
        :return:
        """
    ...
    def parse_config(self, config_dict):
        ...
        for index, regDef in enumerate(config_dict):
            if not regDef.get("Point Name"):
                _log.warning(
                    "Registry configuration contained entry without a point name: {}".format(regDef))
                continue
            ...
            # Create an instance of the register class based on the register
            # type
    ...
    def _set_point(self, point_name, value, **kwargs):
        ...
        if not register.register_type in ["setting", "hold", "vacation", "programs"]:
...
class Setting(BaseRegister):
    ...
    def set_state(self, value):
        ...
        settings_url = 'https://api.ecobee.com/1/thermostat'
        headers = {
            'Content-Type': 'application/json;charset=UTF-8',
            'Authorization': 'Bearer {}'.format(self.access_token)
        }
        ...
        return make_ecobee_request("POST", settings_url, headers, params=params, body=body)
...
class Hold(BaseRegister):
    ...
    def set_state(self, value):
        ...
        :param value: Arbitrarily specified value dictionary. Ecobee API documenation provides best practice information
        for each hold.
        :return: request response values from settings request
        """
        if not isinstance(value, dict):
            raise ValueError("Hold register set_state expects dict, recieved {}".format(type(value)))
        ...
        # Generate set state request content and send request
        hold_url = 'https://api.ecobee.com/1/thermostat'
        headers = {
            'Content-Type': 'application/json;charset=UTF-8',
        ...
        return make_ecobee_request("POST", hold_url, headers, params=params, body=body)
...
class Status(BaseRegister):
    """
    Status request wrapper register for Ecobee theromstats.
    ...
    def get_state(self):
        ...
        status_message = make_ecobee_request("GET", status_url, headers, params=params)
...
class Vacation(BaseRegister):
    """
    Wrapper register for adding and deleting vacations, and getting vacation
    status
    ...
    def set_state(self, vacation, delete=False):
        ...
        if delete:
            ...
            vacation_url = 'https://api.ecobee.com/1/thermostat'
            headers = {
                'Content-Type': 'application/json;charset=UTF-8',
                'Authorization': 'Bearer ' + self.access_token
            }
            ...
            make_ecobee_request("POST", vacation_url, headers, params=params, body=body)
        else:
...
class Program(BaseRegister):
    ...
    def set_state(self, program, resume_all=False):
        ...
        """
        program_url = 'https://api.ecobee.com/1/thermostat'
        headers = {
            'Content-Type': 'application/json;charset=UTF-8',
            'Authorization': 'Bearer ' + self.access_token
        }
        ...
        if not program:
            if not resume_all:
                ...
                _log.info("No program specifed and resume all is set to true, resuming all stored programs.")
        ...
        make_ecobee_request("POST", program_url, headers, params=params, body=body)
...
def make_ecobee_request(request_type, url, headers, params=None, body=None):
    """
    ...
    response = grequests.map([request])[0]
    handle_response_code(response.status_code, response.text)
...
def handle_response_code(response_code, text):
    """
    Make sure response code from Ecobee indicates success
    :param response_code: grequests response code
    :param text: grequests response text mapping to response code
    """
    if response_code == 200:
        return
    else:
