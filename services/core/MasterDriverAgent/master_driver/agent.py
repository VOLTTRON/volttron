...
from .interfaces import venstar_tstat as venstar
...
class MasterDriverAgent(Agent):
    ...
    def _publish_venstar(self):
        v = venstar.Interface()
        venstar_config = "/var/lib/volttron/venstar.config"
        #venstar_config = self.vip.config.get("venstar_config_file")
        with open(venstar_config, "r") as f:
            config_dict = json.load(f)
        v.configure(config_dict, "")
        data = v.get_data()[0]
        topic = f"devices/tva/nashville/cristian_test/hvac0/all"
        peer = "pubsub"
        header = {}
        _log.debug(f"publishing from master driver agent")
        self.vip.pubsub.publish(peer, topic, header, message=data)
