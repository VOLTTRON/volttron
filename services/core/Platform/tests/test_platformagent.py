...
@pytest.fixture
def platform_uuid(volttron_instance1):
    import os
    agent_dir = os.path.abspath(os.path.join(os.path.dirname(__file__),
        os.path.pardir))
    print('AGENT DIR', agent_dir)
    config = os.path.join(agent_dir, "config")
    return volttron_instance1.build_agentpackage(agent_dir, config)
    #return volttron_instance1.direct_build_install_run_agent(agent_dir, config)
