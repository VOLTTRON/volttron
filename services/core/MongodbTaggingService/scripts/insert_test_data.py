...
connection_string = "mongodb://test:test@localhost:27017/mongo_test"
topic_table = "topics"
# device_topics = \
#     "^PNNL/SEB/AHU1/VAV123A/TerminalBoxDamperCommand|^PNNL/SEB/CHWS" \
#     "/HeatExchangeRate|PNNL_SEQUIM/.*"
device_topics = "^PNNL|^campus|^new_campus"

tags_table = "test_tags"
client = MongoClient(connection_string)
mongodb = client.get_default_database()
mongo_bulk = mongodb[tags_table].initialize_ordered_bulk_op()
mongo_batch_size = 0
...
sqlite_connection.execute("CREATE INDEX IF NOT EXISTS idx_topic ON " +
...
cursor = mongodb.topics.find({"topic_name":re.compile(device_topics)}).sort(
    "topic_name")
current_campus= ""
current_site= ""
current_equip= ""
...
#             db_insert(get_equip_tags(parts[0], site,
#                                        None, equip))
#             current_equip = equip
#
#         if num_parts == 5:
...
#             db_insert(get_point_tags(parts[0], site,
#                                        "/".join(parts[:-1]), point))
#             current_point = point
#     else:
...
# now perform test query and time them
start  = datetime.datetime.now()
cursor = mongodb[tags_table].find( {"campus":True, "geoPostalCode" : "20501"},
                          {"_id":1})
refs = [record['_id'] for record in cursor]
print refs
cursor = mongodb[tags_table].find(
    {"campusRef":{"$in":refs}, "equip":True,
     "ahu" : True , "equip_tag 7":{"$gt":2} },
    {"topic_prefix":1 })
topics = [record['_id'] for record in cursor]
print topics
...
'select topic_prefix from test_tags where tag="campusRef" and value IN ( '
'  '
'select value from test_tags where tag="id" and topic_prefix IN ('
'    select topic_prefix from test_tags where tag="campus" and value=1 '
'    INTERSECT '   
'    select topic_prefix  from test_tags where tag="geoPostalCode"  and '
'       value="20501")'
') '
' INTERSECT '
...
' INTERSECT '
'select topic_prefix from test_tags where tag = "equip_tag 7" and value > 2'
)

print ("topics :{}".format(cursor.fetchall()))
