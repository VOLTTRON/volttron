...
def verify_add_task(schedule_manager, agent_id, task_id, requests, priority,
                    now):
    schedule_manager.get_schedule_state(now)

    result = schedule_manager.request_slots(agent_id, task_id, requests,
                                            priority, now)
    schedule_manager.get_schedule_state(now)
...
def test_malformed_schdeule_bad_timestr():
    ...
    sch_man = ScheduleManager(60, now=now)
    ag = ('Agent1', 'Task1',
          (['campus/building/rtu1', 'fdhkdfyug',
