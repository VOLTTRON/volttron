...
class ActuatorAgent(Agent):
    ...
    def _set_point(self, sender, topic, value, point=None, **kwargs):
        ...
        if self._check_lock(path, sender):
            ...
            raise LockError("caller ({}) does not have lock for point {} on device {}".format(sender, point_name, path))
    ...
    def _revert_device(self, sender, topic, **kwargs):
        ...
        if self._check_lock(path, sender):
            ...
            raise LockError("caller ({}) does not have lock for device {}".format(sender, path))
