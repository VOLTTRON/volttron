...
ENV_POSTGRESQL_WITH_DB = {
    "POSTGRES_USER": ROOT_USER,  # defining user not necessary but added to be explicit
    "POSTGRES_PASSWORD": ROOT_PASSWORD,
    "POSTGRES_DB": TEST_DATABASE,
}
VOLUMES_TEST_DB = {
    "".join([os.getcwd(), "/init_test_db"]): {
        "bind": "/docker-entrypoint-initdb.d",
        "mode": "ro",
    }
}
VOLUMES_METADATA_ONLY = volume = {
    "".join([os.getcwd(), "/init_metadata_only"]): {
        "bind": "/docker-entrypoint-initdb.d",
        "mode": "ro",
    }
}
...
def test_setup_historian_tables_should_create_tables(get_container_func, ports_config):
    ...
    with get_container(
        image, ports=ports_config["ports"], env=ENV_POSTGRESQL_WITH_DB
    ) as container:
...
def test_read_tablenames_from_db_should_return_table_names(
    ...
    with get_container(
        image,
        ports=ports_config["ports"],
        env=ENV_POSTGRESQL,
        volumes=VOLUMES_METADATA_ONLY,
    ) as container:
        port_on_host = ports_config["port_on_host"]
        wait_for_connection(container, port_on_host)
...
@pytest.mark.parametrize(
    ...
    [
        ([24], {24: "topic24"}, {"topic24": []}),
        (
            [13],
            {13: "topic13"},
            {"topic13": [("2020-06-01T12:30:59.000000+00:00", [2, 3])]},
        ),
    ],
)
@pytest.mark.postgresqlfuncts
def test_query_should_return_data(
    ...
    get_container, image = get_container_func
    with get_container(
        image, ports=ports_config["ports"], env=ENV_POSTGRESQL, volumes=VOLUMES_TEST_DB
    ) as container:
        port_on_host = ports_config["port_on_host"]
        wait_for_connection(container, port_on_host)
...
def test_insert_topic_should_return_topic_id(get_container_func, ports_config):
    ...
    with get_container(
        ...
        with get_postgresqlfuncts(port_on_host) as postgresqlfuncts:
            ...
            assert isinstance(actual_topic_id, int)
...
def test_insert_agg_topic_should_return_agg_topic_id(get_container_func, ports_config):
    ...
    with get_container(
        ...
        with get_postgresqlfuncts(port_on_host) as postgresqlfuncts:
            topic = "foobar_topic"
            agg_type = "SUM"
            agg_time_period = "1989"

            actual_id = postgresqlfuncts.insert_agg_topic(
                topic, agg_type, agg_time_period
            )

            expected_data = [(actual_id, topic, agg_type, agg_time_period)]
            assert isinstance(actual_id, int)
            assert (
                get_data_from_aggregate_by_topic_id(
                    port_on_host, AGG_TOPICS_TABLE, actual_id
                )
                == expected_data
            )
...
def test_insert_meta_should_return_true(get_container_func, ports_config):
    ...
    with get_container(
        ...
        with get_postgresqlfuncts(port_on_host) as postgresqlfuncts:
            ...
            assert res is True
            assert get_data_(port_on_host, "meta")[0] == expected_data
...
def test_insert_data_should_return_true(get_container_func, ports_config):
    ...
    with get_container(
        ...
        with get_postgresqlfuncts(port_on_host) as postgresqlfuncts:
            ...
            assert res is True
            assert (
                expected_data == get_data_by_topic_id(port_on_host, "data", topic_id)[0]
            )
...
def test_update_topic_should_return_true(get_container_func, ports_config):
    ...
    with get_container(
        ...
        with get_postgresqlfuncts(port_on_host) as postgresqlfuncts:
            ...
            expected_data = (actual_id, "soccer")
            ...
            assert (
                get_data_by_topic_id(port_on_host, "topics", actual_id)[0]
                == expected_data
            )
...
def test_update_agg_topic_should_return_true(get_container_func, ports_config):
    ...
    with get_container(
        ...
        with get_postgresqlfuncts(port_on_host) as postgresqlfuncts:
            ...
            assert isinstance(actual_id, int)
            expected_data = [(actual_id, "cars", "SUM", "2100ZULU")]
            assert (
                get_data_from_aggregate_by_topic_id(
                    port_on_host, AGG_TOPICS_TABLE, actual_id
                )
            ...
            expected_data = [(actual_id, "boats", "SUM", "2100ZULU")]
...
def test_insert_agg_meta_should_return_true(get_container_func, ports_config):
    ...
    with get_container(
        ...
        with get_postgresqlfuncts(port_on_host) as postgresqlfuncts:
            ...
            assert (
                get_data_from_aggregate_by_topic_id(
                    port_on_host, AGG_META_TABLE, topic_id
                )
                == expected_data
            )
...
def test_get_topic_map_should_return_maps(get_container_func, ports_config):
    ...
    with get_container(
        ...
        with get_postgresqlfuncts(port_on_host) as postgresqlfuncts:
            expected = (
                {"baseball": 2, "football": 1, "foobar": 3, "xctljglfkjsgfklsd": 4},
                {
                    "baseball": "baseball",
                    "football": "football",
                    "xctljglfkjsgfklsd": "xctljglfkjsgfklsd",
                    "foobar": "foobar",
                },
            )
...
def test_get_agg_topic_map_should_return_dict(get_container_func, ports_config):
    ...
    with get_container(
        ...
        with get_postgresqlfuncts(port_on_host) as postgresqlfuncts:
            expected_dict = {("some_agg_topic", "AVG", "2019"): 1}

            actual_dict = postgresqlfuncts.get_agg_topic_map()
...
def test_query_topics_by_pattern_should_return_matching_results(
    ...
    with get_container(
        ...
        with get_postgresqlfuncts(port_on_host) as postgresqlfuncts:
            expected_topics = {"football": 1, "foobar": 3}
            topic_pattern = "foo"

            actual_topics = postgresqlfuncts.query_topics_by_pattern(topic_pattern)
...
@pytest.mark.postgresqlfuncts
def test_insert_aggregate_stmt_should_return_true(get_container_func, ports_config):
    get_container, image = get_container_func
    ...
    with get_container(
        ...
        with get_postgresqlfuncts(port_on_host) as postgresqlfuncts:
            ...
            assert res is True
            assert get_data_(port_on_host, "avg_1776") == expected_data
...
def get_data_by_topic_id(port, table, topic_id):
    cnx, cursor = get_cnx_cursor(port)
    query = SQL("SELECT * " "FROM {table_name}" "WHERE topic_id = %s").format(
        table_name=Identifier(table)
    )

    return execute_statement(cnx, cursor, query, args=[topic_id])


def get_data_from_aggregate_by_topic_id(port, table, topic_id):
    cnx, cursor = get_cnx_cursor(port)
    query = SQL("SELECT * " "FROM {table_name}" "WHERE agg_topic_id = %s").format(
        table_name=Identifier(table)
    )
