...
def master_web_service():
    serverkey = "serverkey"
    mock_aip = mock.Mock()
    yield PlatformWebService(serverkey=serverkey, identity=MASTER_WEB, address="tcp://stuff",
                             bind_web_address="http://v2:8888")


@contextlib.contextmanager
def get_master_web(bind_web_address="http://v2:8080", **kwargs) -> PlatformWebService:
    """
    ...
    mws = PlatformWebService(serverkey=serverkey, identity=MASTER_WEB, address="tcp://stuff",
...
def test_discovery(scheme):
    ...
    with get_test_volttron_home(messagebus='zmq', config_params=config_params):
        ...
        with mock.patch('volttron.platform.vip.agent.subsystems.query.Query', _construct_query_mock):
            ...
            mws = PlatformWebService(serverkey=serverkey, identity=MASTER_WEB, address=address,
...
@pytest.mark.web
def test_register_route(master_web_service: PlatformWebService):
...
@pytest.mark.web
def test_register_endpoint(master_web_service: PlatformWebService):
    ws = master_web_service
    fn_mock = mock.Mock()
