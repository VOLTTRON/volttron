...
@bacpypes_debugging
class SynchronousApplication(BIPForeignApplication):
...
def main():
    ...
    with open(args.device_map) as f:
        device_map = jsonapi.load(f)
    found_networks = defaultdict(list)
    # make a simple application
    for bbmd in device_map:
        this_application = SynchronousApplication(this_device, args.ini.address, Address(bbmd), 10)
        # this_application = WhoIsIAmApplication(this_device, args.ini.address, Address(bbmd), int(10))
        for network, devices in device_map[bbmd].items():
            for device in devices:
                device_address, device_id = device.split('-')
                _log.info(f"trying to find {device_id} on {network} via {bbmd}")
                try:
                    device_name = read_prop(this_application, Address(device_address), "device", int(device_id), "vendorIdentifier")
                    if device_name:
                        found_networks[bbmd].append((network, device_address, device_id))
                        break
                except TypeError:
                    _log.info(f"No objectName found for {device_id} on {network}")
            if network in [entry[1] for entry in found_networks]:
                continue
        this_application.close_socket()
        sleep(300)
    
    with open(args.bbmd_map, 'w') as f:
        jsonapi.dump(found_networks, f)
