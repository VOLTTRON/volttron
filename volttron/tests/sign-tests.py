...
class TestAuth(unittest.TestCase):
    ...
    def setUpClass(cls):
        if not os.path.exists(KEY_DIR):
            os.makedirs(KEY_DIR)
            assert(create_self_signed_cert(KEY_DIR, KEY_ROOT) is True)
    ...
    def test_unsigned_wheel(self):
        ...
        try:
            ...
            with self.assertRaises(KeyError) as context:
                verify_wheel_signature(
                    wheel_copy, join(KEY_DIR, PUBLIC_KEY_NAME))
    ...
    def test_wrong_key(self):
        ...
        try:
            ...
            self.assertTrue(
                sign_wheel(wheel_copy, join(KEY_DIR, PRIVATE_KEY_NAME)))
            self.assertTrue(
                verify_wheel_signature(wheel_copy, join(KEY_DIR, PUBLIC_KEY_NAME)))

            with self.assertRaises(Exception) as context:
                verify_wheel_signature(
                    wheel_copy, join(KEY_DIR, other_key_root + '.crt'))
