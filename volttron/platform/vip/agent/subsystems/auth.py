...
from volttron.platform.agent.utils import get_platform_instance_name, \
    get_fq_identity, get_messagebus
...
class Auth(SubsystemBase):
    ...
    def request_cert(self, csr_server, fully_qualified_local_identity,
        ...
        if status == 'SUCCESSFUL' or status == 'APPROVED':
            certs.save_agent_remote_info(remote_certs_dir,
                                         ...
                                         cert.encode("utf-8"),
                                         remote_ca_name,
                                         discovery_info.rmq_ca_cert.encode(
                                             "utf-8"))
            os.environ['REQUESTS_CA_BUNDLE'] = os.path.join(remote_certs_dir,
                                                            "requests_ca_bundle")
            _log.debug("Set os.environ requests ca bundle to {}".format(
    ...
    def set_rpc_authorizations(self, method_str, capabilities):
        ...
        _log.debug(
            f"Authorized capabilities: {capabilities} for method: "
            f"{method_str} set")
    ...
    def update_rpc_method_capabilities(self):
        ...
        if rpc_method_authorizations != updated_rpc_method_authorizations:
            for method in updated_rpc_method_authorizations:
                self.set_rpc_authorizations(method,
                                            updated_rpc_method_authorizations[
                                                method])
