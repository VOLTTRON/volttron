...
def _create_shovel_setup(instance_name, local_host, port, vhost, vhome, is_ssl):
    ...
    ssl_params = None
    print("shovel config: {}".format(shovel_config))
    try:
        for remote_host, shovel in shovels.items():
            pubsub_config = shovel.get("pubsub", {})
            print("shovel parameters: {}".format(shovel))
            for identity, topics in pubsub_config.items():
                ...
                if 'destination' in shovel:
                    print("shovel parameters under destination: {}".format(shovel))
                    ...
                    if is_csr:
                        ...
                        print("certs parameters: {}".format(certs_dict))
                ...
                if not isinstance(topics, list):
                    topics = [topics]
                for topic in topics:
                    print("Creating shovel to forward PUBSUB topic {}".format(
                        topic))
...
def prompt_shovels(vhome):
    ...
    try:
        for i in range(0, count):
            ...
            print("shovel_user: {}".format(shovel_user))
            ...
            if prompt in y:
                ...
                # ca cert
                shovels[host]['destination']['ca_cert'] = ca_file
                    ...
                    if response[0] == 'PENDING':
                        while not success and retry_attempt < 3:
                            ...
                            if response is None:
                                ...
                                time.sleep(20)
                                print("Attempting CSR for shovel: {} again".format(shovel_user))
                                retry_attempt += 1
                            else:
                                success = True
                else:
                ...
                if success:
                    print("Received successful response to csr request")
                    ...
                    print("shovel ca file path: {}".format(ca_file))
                    #certs.Certs.cert_file(metadata['remote_ca_name'], remote=True)
                    shovels[host]['destination']['remote_ca'] = ca_file

                    # public cert
                    shovels[host]['destination']['public_cert'] = certfile
                    print("shovel public cert path: {}".format(certfile))

                    # private_key
                    crts = certs.Certs()
                    shovels[host]['destination']['private_cert'] = prvtfile = crts.private_key_file(name=local_keyfile)
                    print("shovel prvtfile path: {}".format(prvtfile))

                    print("Shovel config with csr certs: {}".format(shovels))
...
def request_shovel_cert(shovel_user, csr_server, fully_qualified_local_identity, discovery_info):
    ...
    crts = certs.Certs()
    crts.create_signed_cert_files(fully_qualified_local_identity, cert_type='client')
