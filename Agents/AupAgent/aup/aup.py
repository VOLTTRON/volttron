...
def display_matrix(m, labelstring, xlabels, ylabels, rowstring):
    # Display header
    print labelstring % tuple(xlabels)

    i = 0
    while i < len(ylabels):
        print rowstring % ((ylabels[i],) + tuple(m[i]))
        i += 1
...
def normalize_matrix(m, col_sums):
    ...
    while i < len(m):
        ...
        norm_row = []
        while j < len(m[0]):
            norm_row.append(m[i][j]/col_sums[j])
            j += 1
        rowsum = sum(norm_row)
...
def demo_aup(criteria_matrix, device_matrix, devices, criteria_labels="", criteria_labelstring="", matrix_rowstring=""):
    print "Criteria Matrix"
    display_matrix(criteria_matrix, criteria_labelstring, criteria_labels, criteria_labels, matrix_rowstring)
    ...
    display_matrix(normalized_matrix, criteria_labelstring, criteria_labels, criteria_labels, matrix_rowstring)
    ...
    display_matrix([device_csums, ], criteria_labelstring, criteria_labels, ['Device Criteria Sums'], matrix_rowstring)
    print ""
    print "Normalized device matrix"
    display_matrix(device_normalized_matrix, criteria_labelstring, criteria_labels, devices, matrix_rowstring)
...
def do_aup(criteria_matrix, device_matrix, devices):
    # Calculate the column sums for use in validating and normalizing the input matrix
    col_sums = calc_column_sums(criteria_matrix)
...
def validate_input(pairwise_matrix, col_sums, display=False, criteria_labels="", criteria_labelstring="", matrix_rowstring=""):
    ...
    display_matrix([roots, ], criteria_labelstring, criteria_labels, ['5th root of product'], matrix_rowstring) if display else ""
    ...
    display_matrix([priority_vec, ], criteria_labelstring, criteria_labels, ['Priority Vector'], matrix_rowstring) if display else ""
    ...
    display_matrix([priority_row, ], criteria_labelstring, criteria_labels, ['Priority Row'], matrix_rowstring) if display else ""

    # Sum the priority row
    priority_row_sum = sum(priority_row)

    if display:
        print "Priority row sum: ", priority_row_sum
