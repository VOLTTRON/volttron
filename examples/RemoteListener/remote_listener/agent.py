...
class ListenerAgent(Agent):
    ...
    # def request_remote_certificate(self):
    #     _log.debug("Requesting remote certificate")
    #     self.vip.auth.request_cert("http://localhost:8080")
    ...
    def _connect_to_remote(self):
        ...
        if not self._remote_agent:
            ...
            self._remote_agent.vip.pubsub.publish('pubsub', topic='bar', message="{foo}").get()
            self._remote_agent.vip.pubsub.subscribe('pubsub', prefix='devices', callback=self.on_remote_match)

    def on_remote_match(self, peer, sender, bus,  topic, headers, message):
